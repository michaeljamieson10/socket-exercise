...
class Room {
  ...
  toSelf(member){
    member.send(JSON.stringify({type: "chat",
    text: "What do you call eight hobbits? A hob-byte!",
    name: "Server"}));
  }
  toMember(fromMemb, toMemb, msgText){
    console.log(this.members, '<--- these are members')
    for (let member of this.members) {
      if(member.name === toMemb || member.name === fromMemb){
        member.send(JSON.stringify({type: "chat",
        text: msgText,
        name: fromMemb}))
      }
